...
//                         start: new Date(event.startDate), // ISO 형식으로 변환
//                         end: new Date(event.endDate),    // ISO 형식으로 변환
//                         color: event.color,
...
//         // 시작일과 종료일을 동일하게 설정하여 하루 일정으로 처리
//         const adjustedEndDate = new Date(startDate);
//         adjustedEndDate.setHours(23, 59, 59, 999); // 종료 날짜를 시작일의 마지막 순간으로 설정

//         dispatch(addEvent({ calendarId, title, startDate, endDate: adjustedEndDate.toISOString(), memo }))
//             .unwrap()
//             .then((newEvent) => {
//                 console.log("새로 추가된 이벤트:", newEvent);
                
//                 // 백엔드에서 반환된 이벤트 데이터에 color와 allDay가 없을 경우 추가
//                 const adjustedEvent = {
//                     ...newEvent,
...
//         if (selectedEvent) {
//             if (title && startDate) {
//                 // 종료 날짜를 시작일의 마지막 순간으로 설정하여 하루 일정으로 처리
//                 const adjustedEndDate = new Date(startDate);
//                 adjustedEndDate.setHours(23, 59, 59, 999); // 종료 날짜를 시작일의 마지막 시간으로 설정
...
//                     calendarName, // 선택된 캘린더로 업데이트
//                     start: startDate,
//                     end: adjustedEndDate.toISOString(), // 수정된 종료일 반영
//                     memo,
//                     color: eventColor, // 새로 선택된 캘린더의 색상으로 업데이트
//                     allDay: true,
//                 };
...
// })
// .catch((error) => {
//     console.error('이벤트 수정 에러:', error.message);
//     console.error('오류 전체 내용:', error);
// });
//             } else {
//                 alert('제목, 캘린더 이름, 시작 날짜를 모두 입력해야 합니다.');
//             }
...
//                 {/* 일정 추가 및 수정 모달 */}
//                 <Modal isOpen={addEventModalIsOpen} onRequestClose={closeModal} style={eventModalStyles}>
