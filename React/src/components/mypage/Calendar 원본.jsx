...
//---------------------------------------------------------------------------------------------------------------------------------------------
        ...
        const loadCalendars = async () => {
            try {
                ...
                if (Array.isArray(calendarsData)) {
                    ...
                    setCalendarOptions([]); // 배열이 아닐 경우 빈 배열로 설정
                }
            } catch (error) {
                console.error("캘린더 로드 에러:", error);
                setCalendarOptions([]); // 에러 발생 시에도 빈 배열로 설정
            }
        };
        loadCalendars();
    }, []);
    ...
    useEffect(() => {
        if (events && events.length > 0) {
            console.log("이벤트 리스트 업데이트됨:", events);
            // 여기에 캘린더 컴포넌트에 이벤트를 추가하는 로직을 추가하거나,
            // 필요하다면 다른 작업을 수행할 수 있습니다.
        }
    }, [events]);
    ...
    //         // 기존 이벤트의 시작 및 종료 날짜를 업데이트 (상태에 저장될 값)
    ...
    const renderEvents = () => {
        return events.map(event => {
            ...
            if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
                console.error('유효하지 않은 날짜 형식입니다:', event.start, event.end);
                return null; // 유효하지 않은 이벤트는 제외
            }
    
            // 렌더링 시에만 종료 날짜에 하루 추가
            ...
            return {
                ...
                end: renderedEndDate.toISOString().split('T')[0], // 하루 추가된 종료일로 설정
            };
        }).filter(event => event !== null); // 유효하지 않은 이벤트는 필터링
    };
