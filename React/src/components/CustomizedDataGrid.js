...
// 특정 ID의 폴더를 부모 폴더에서 제거하는 함수
const removeFolderById = (folder, targetId) => {
  if (folder.children) {
    for (let i = 0; i < folder.children.length; i++) {
      if (folder.children[i].id === targetId) {
        return folder.children.splice(i, 1)[0]; // 폴더 제거 후 반환
      }
      const result = removeFolderById(folder.children[i], targetId);
      if (result) {
        return result;
      }
    }
...
const handleFileUpload = (event) => {
  ...
  const folderId = selectedFolder.id; // 선택된 폴더의 ID
