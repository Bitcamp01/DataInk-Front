...
export default function CustomizedTreeView({ folderData,setSelectedFolder,setFolderData,setFlatFolderData,handleTreeViewMouseEnter }) {
  ...
  const createNewProject = (e) => {
    // 백엔드에 요청을 보내고 newRootFolder정보를 받는다.

    const response="ok"
    // 요청이 성공 했을 때 클라이언트 단에서 보관하고 있는 데이터를 수정
    // 응답에 새로 생성되는 요소의 아이디 값은 가져와야 할 듯
    if(response == "ok" ){
      //newRootFolder정보를 백엔드 서버로부터 받음, 이름,설명 정도만 가져감 반환 폴더 정보에는 설명 정보는 없음
      const newRootFolder = {
        id: new Date().getTime().toString(), // 백엔드에서 받은 고유한 ID 값
