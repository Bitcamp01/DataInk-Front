...
export default function CustomizedDataGrid({getSelectedFolderData,folderData,setSelectedProject,getInitFolderData,selectedProject,selectedFolder = null,setSelectedFolder,flatFolderData,setFlatFolderData}) {
  ...
  const handleOpenItemModal = async () => {
    try {
      ...
      const response = await axios.get('https://223.130.134.24:9090/item'); // 예시: 실제 API 엔드포인트 사용
      if (!response.ok) {
...
const handleFileUpload = async (event) => {
  ...
  try {
    const response= await axios.post('https://223.130.134.24:9090/project/uploadfile', selectedFiles,{
      params: {
...
const handleCopy = () => {
  ...
  const processRowUpdate = async(newRow) => {
    ...
    try {
      ...
      const response = await axios.post(`https://223.130.134.24:9090/projects/rename`, {
        label: newRow.label,
  ...
  const handleCreateNewFolder = async () => {
    try {
      const response = await axios.post(
          "https://223.130.134.24:9090/projects/createfolder",
          {
            selectedFolder: selectedFolder,
            selectedProject: selectedProject,
          },
  ...
  const handleDelete = async (e) => {
    ...
    if (rowSelectionModel.length > 0) {
      try {
        const response = await axios.post('https://223.130.134.24:9090/projects/delete', {
      ...
      <Dialog open={isConversionModalOpen} onClose={handleCloseConversionModal} fullWidth maxWidth="sm">
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={ async () => {
            ...
            const response = await axios.post("https://223.130.134.24:9090/projects/conversion",conversionList,{
              'Authorization': `Bearer ${sessionStorage.getItem('ACCESS_TOKEN')}`,
            })
