...
export const deleteProfileImage = createAsyncThunk(
    ...
    async (_, thunkApi) => {
        try {
            const response = await axios.delete('http://localhost:9090/mypage/profile-image', {
                headers: {
                    'Authorization': `Bearer ${sessionStorage.getItem('ACCESS_TOKEN')}`
                }
            });
...
export const deleteBackgroundImage = createAsyncThunk(
    ...
    async (_, thunkApi) => {
        try {
            const response = await axios.delete('http://localhost:9090/mypage/background-image', {
                headers: {
                    'Authorization': `Bearer ${sessionStorage.getItem('ACCESS_TOKEN')}`
                }
            });
