    ...
    async (password, thunkApi) => {
        try {
            const response = await axios.post(
                'https://223.130.134.24/mypage/password-check',
                { password },
                {
                    headers: {
    ...
    async (userInfo, thunkApi) => {
        try {
            const response = await axios.put(
                'https://223.130.134.24/mypage/update-profile',
                userInfo,
                {
                    headers: {
                        'Authorization': `Bearer ${sessionStorage.getItem('ACCESS_TOKEN')}`,
...
export const getAllProjects = createAsyncThunk(
    ...
    async (_, thunkApi) => {
        try {
            const response = await axios.get('https://223.130.134.24/projects/all', {
                headers: {
                    Authorization: `Bearer ${sessionStorage.getItem('ACCESS_TOKEN')}`,
                },
            });
...
export const fetchMypageInfo = createAsyncThunk(
    ...
    async (_, thunkApi) => {
        try {
            const response = await axios.get('https://223.130.134.24/mypage', {
                headers: {
                    'Authorization': `Bearer ${sessionStorage.getItem('ACCESS_TOKEN')}`,
                }
            });
...
export const deleteProfileImage = createAsyncThunk(
    ...
    async (_, thunkApi) => {
        try {
            const response = await axios.delete('https://223.130.134.24/mypage/profile-image', {
                headers: {
                    'Authorization': `Bearer ${sessionStorage.getItem('ACCESS_TOKEN')}`
                }
            });
...
export const deleteBackgroundImage = createAsyncThunk(
    ...
    async (_, thunkApi) => {
        try {
            const response = await axios.delete('https://223.130.134.24/mypage/background-image', {
                headers: {
                    'Authorization': `Bearer ${sessionStorage.getItem('ACCESS_TOKEN')}`
                }
            });
