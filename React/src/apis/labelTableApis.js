...
export const fetchFolders = createAsyncThunk(
    ...
    async (projectId, thunkApi) => {
      try {
        const response = await axios.get(`http://223.130.134.24:9090/projects/${projectId}/folders`, {
          headers: {
...
export const fetchTasksByFolderId = createAsyncThunk(
  ...
  async (folderId, { rejectWithValue }) => {
    try {
      const response = await axios.get(`http://223.130.134.24:9090/mongo/tasks/folder/${folderId}`, {
        headers: {
          Authorization: `Bearer ${sessionStorage.getItem('ACCESS_TOKEN')}`
        }
      });
...
export const fetchProjectEndDate = createAsyncThunk(
  ...
  async (projectId, { rejectWithValue }) => {
    try {
      const response = await axios.get(`http://223.130.134.24:9090/projects/enddate/${projectId}`, {
        headers: {
          Authorization: `Bearer ${sessionStorage.getItem('ACCESS_TOKEN')}`
        }
      });
...
export const submitForReview = createAsyncThunk(
  ...
  async (taskIds, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.put(`http://223.130.134.24:9090/mongo/tasks/update-submit`, taskIds, {
                headers: {
                    Authorization: `Bearer ${sessionStorage.getItem('ACCESS_TOKEN')}`,
                },
            });
