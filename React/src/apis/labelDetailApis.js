...
export const saveLabelTask = createAsyncThunk(
    'labelTask/saveLabelTask',
    async (obj, { rejectWithValue }) => {
        console.log("!11111111111111111111111111111111");
        console.log(obj);
      try {
        const { taskId, transformedData } = obj; // 각각의 값 분리
        const response = await axios.patch(`${API_BASE_URL}/labeltask/labelDetailsSave`, { // 이 부분도 컨트롤러 만들면 바꿔라
...
export const fetchLabelTaskDetails = createAsyncThunk(
    ...
    async (taskId, { rejectWithValue }) => {
        ...
        console.log(taskId);
      try {
        const response = await axios.get(`${API_BASE_URL}/labeltask/labelDetails/${taskId}`, {
          headers: AUTH_HEADER,
        });
        return response.data;
      } catch (error) {
